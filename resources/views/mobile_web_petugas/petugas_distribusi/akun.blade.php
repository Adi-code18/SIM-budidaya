...
<div class="p-4 space-y-4" x-data="akunPetugasDistribusi()">
        ...
        <button @click="triggerToast(i18n[currentLang].notifSaved, 'info')"
            ...
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 font-medium" x-text="i18n[currentLang].notificationsSub">Suara, alert pengiriman & push notify</p>
                </div>
            </div>
            <i class="fa-solid fa-chevron-right text-slate-400 text-xs"></i>
        </button>

        <!-- Option 2: Bantuan & Dukungan -->
        <a href="https://wa.me/6281234567890?text=Halo%20Admin%20SIM-BUDIDAYA,%20saya%20butuh%20bantuan%20sistem." target="_blank"
           class="w-full p-4 flex items-center justify-between hover:bg-slate-50 transition-colors text-left">
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 font-medium" x-text="i18n[currentLang].helpSub">FAQ, Panduan app & CS Support</p>
                </div>
            </div>
            <i class="fa-solid fa-chevron-right text-slate-400 text-xs"></i>
        </a>
    ...
    <div x-show="editProfileModal" 
        ...
        <div class="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl p-5 space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                <h3 class="text-xs font-bold text-navy-900" x-text="i18n[currentLang].editProfile">Edit Profile Petugas</h3>
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input type="text" x-model="userName" class="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-800 focus:outline-none focus:border-navy-800">
                </div>
                <div>
                    <label class="text-[10px] font-extrabold uppercase text-slate-500 block mb-1" x-text="i18n[currentLang].phone">NOMOR HANDPHONE</label>
                    <input type="text" x-model="userPhone" class="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-800 focus:outline-none focus:border-navy-800">
                </div>
                <div>
    ...
    <!-- Modal 2: Language Picker Modal (Hanya Indonesia & English) -->
...
@push('scripts')
<script>
function akunPetugasDistribusi() {
    return {
        editProfileModal: false,
        langModal: false,
        logoutModal: false,
        userName: '{{ Auth::user()->nama ?? 'Petugas Distribusi' }}',
        userPhone: '{{ Auth::user()->no_tlp ?? '+62 812-3456-7891' }}',
        currentLang: localStorage.getItem('sim_lang') || 'id',

        i18n: {
            id: {
                role: 'Petugas Logistik & Distribusi',
                editBtn: 'Edit Profil',
                editProfile: 'Edit Profil Petugas',
                ...
                notificationsSub: 'Suara, alert pengiriman & push notify',
                notifSaved: 'Pengaturan notifikasi berhasil disimpan',
                help: 'Bantuan & Dukungan',
                helpSub: 'FAQ, Panduan app & CS Support',
                language: 'Bahasa',
            ...
            en: {
                role: 'Logistics & Distribution Officer',
                editBtn: 'Edit Profile',
                editProfile: 'Edit Officer Profile',
                fullName: 'FULL NAME',
                phone: 'PHONE NUMBER',
                jobTitle: 'POSITION',
                ...
                notificationsSub: 'Sound, delivery alerts & push notifications',
                notifSaved: 'Notification settings saved successfully',
                help: 'Help & Support',
                helpSub: 'FAQ, App Guide & CS Support',
                language: 'Language',
