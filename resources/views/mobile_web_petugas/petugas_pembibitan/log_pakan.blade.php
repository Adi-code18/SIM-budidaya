...
<div class="p-4 space-y-4" x-data="{
    kolam: 'Tank Hatchery H-01',
    waktuTanggal: '2026-08-20T08:30',
    peletKg: 3.0,
    daunKg: 0.0,
    hargaPerKg: 15000,
    phAir: 7.0,
    ...
    handleSave() {
        ...
        triggerToast('Log pakan harian untuk ' + this.kolam + ' berhasil disimpan!', 'success');
        setTimeout(() => {
    ...
    <div class="bg-white rounded-3xl border border-slate-200/90 p-4 shadow-xs">
        ...
        <p class="text-xs text-slate-500 font-medium mt-0.5">Catat pemberian pakan dan kondisi air di kolam hatchery.</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/90 p-5 shadow-xs">
        ...
        <form @submit.prevent="handleSave()" class="space-y-4">
            ...
            <!-- Field 3: Detail Pakan Grid (Pelet kg & Daun kg) -->
            <div class="space-y-2 pt-1">
                <div class="flex items-center gap-2 text-xs font-bold text-slate-900">
                    <i class="fa-solid fa-wheat-awn text-sky-600"></i>
            ...
            <!-- Field 4: Kebutuhan Pakan (RP) -->
            <div class="space-y-1">
                <label class="text-[10px] font-extrabold uppercase text-slate-500 tracking-wider block">KEBUTUHAN PAKAN (RP)</label>
                <div class="px-3.5 py-2.5 rounded-2xl border border-slate-200 bg-slate-50 flex items-center justify-between text-xs">
                    ...
                    <span class="font-extrabold text-navy-900 text-sm" x-text="totalBiaya.toLocaleString('id-ID')">45.000</span>
                </div>
            </div>

            <!-- Field 5: pH Air Parameter -->
